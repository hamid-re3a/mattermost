...
class Reserve extends React.Component {
    ...
    renderStatus(is_accepted, is_paid, reserve) {
        ...
        return <View style={{ flexDirection: 'row', alignItems: 'center' }}>
            <Button danger >
                <TText style={{ fontFamily: 'IRANSansMobile' }}>کنسل کن</TText>
            </Button>
        </View>;
    }
            ...
            <CardItem style={{ borderBottomWidth: 1, borderBottomColor: '#ddd' }}>
                <Left>
                    ...
                    <Body style={{ alignItems: 'center' }}>
                        <TText style={{ marginRight: 20, fontFamily: 'IRANSansMobile' }}>{item.user.first_name}</TText>
    ...
    async getReservesRequest() {
        try {
            ...
            let response = await fetch(`http://server.reservina.ir/no_answer_reserves?page=${page}`, {
                method: 'GET',
                headers: {
                    'Accept': 'application/json',
                    'Content-Type': 'application/json',
    ...
    renderEmptyList() {
        return <Content>
            <Card>
                <CardItem header>
                    <TText style={{ fontFamily: 'IRANSansMobile' }}>رزروهای بی‌پاسخ</TText>
                </CardItem>
                <CardItem>
                    <Body>
                        <TText style={{ fontFamily: 'IRANSansMobile' }}>
                            در این بخش شما میتوانید لیست رزروهای بی پاسخ
            ...
            <Container>
                <Header style={{ backgroundColor: '#DC3023' }} androidStatusBarColor="#8F1D21" iosBarStyle="light-content">
                    ...
                    <Body style={[general.title, { flex: 2 }]}>
                        <Title style={[general.text, { color: 'white' }]}>رزروهای بی‌پاسخ</Title>
                    </Body>
                    <Right style={{ flex: 1 }}>
